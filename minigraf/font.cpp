...
void drawFontString(String s, int scale)
{
	...
	for (int i = 0; i < s.length(); i++)
	{
		...
		Point savedpos = getCurrentPlotterPos();
		savedpos.y += scale; //jeden bod nad carou
		//LOG(" saved pos:"+String(savedpos.x)+","+String(savedpos.y));
        if (ch < ' ' || ch > '\x7f')
        {
            continue;
        }
        if (ch != ' ')
        {
			...
			//LOG(" letter offset:"+String(pos));
			do
			{				
				//LOG("  instr "+String(characters[pos]));
                byte instr = characters[pos] & 0x7f;
                if ((instr & 0x40) > 0)
                {
                    if (penUp)
                    {
				...
				nx*=scale;
				ny*=scale;
				MOVETO(savedpos.x + nx, savedpos.y + ny);
			} while(!isFontCharEnd(characters[pos++]));
        }
        //move
        penUp = true;
		PU();
		MOVETO((6 * scale) + savedpos.x, savedpos.y - scale);
    }
};
